...
class Trigon:
    """
    Класс для проверки треугольника.
    При инициализации происходит проверка переданных данных и генерируются следующие исключения:
    Если хотя бы одна сторона передана не числом - падаем с TypeError и текстом 'Стороны должны быть числами';
    Если хотя бы одна сторона передана нулем или отрицательным числом -
    падаем с ValueError и текстом 'Стороны должны быть положительными';
    Если не соблюдается неравенство треугольника - Exception и текст "Не треугольник";
    Если передано не 3 аргумента - IndexError "Передано {n} аргументов, а ожидается 3", где n - кол-во аргументов
    """

    def __init__(self, *arg):
        """
        Функция проверяет треугольник по переданным данным длин его сторон.
