...
class PersonInfo:
    ...
    def short_name(self):
        """
        Метод возвращает сокращенную строку с фамилией и именем.
        :return: сокращенно "Фамилия И."
        """
        return self.name[self.name.index(' ') + 1:] + ' ' + self.name[:self.name.index(' ')][0] + '.'

    def path_deps(self):
        """
        Метод возвращает путь по иерархии подразделений.
    ...
    def new_salary(self):
        """
        Метод вычисляет новую индексированную зарплату по формуле.
        :return: 1337*возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        """
