...
try:
    ...
    add_btn.click()
    sleep(10)
    search = driver.find_element(By.CSS_SELECTOR, '.controls-StackTemplate-content [name="ws-input_2023-06-13"]')
    assert search.is_displayed(), 'Поле поиска не отображается'
    search.send_keys('Кай', Keys.ENTER)
    sleep(1)
    ...
    text_box.send_keys('Шурима')
    assert text_box.text == 'Шурима', 'Сообщение в поле отличается от введенного'
    ...
    assert list_msg[0].text.find('Шурима') != -1, \
    ...
    del_msg.click()
    sleep(3)
    assert list_msg[0].text.find('Шурима') == -1, 'Сообщение не удалено'
